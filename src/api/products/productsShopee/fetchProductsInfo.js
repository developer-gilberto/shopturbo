...
export async function fetchProductsInfo(itemIdList) {
  ...
  try {
    const url = encodeURI(
      `${process.env.SERVER_URL}/api/shopee/shop/${shopturboShopId}/products/full-info?item_id_list=${itemIdList}`,
    );
    ...
    if (!response.ok)
      return {
        ...
        message:
          'There was a problem trying to retrieve product information in fetchProductsInfo()',
        data: null,
      };
